<div class="row-fluid">
<div class="span12">
<?php
$this->breadcrumbs=array(
	UserModule::t('Users')=>array('admin'),
	$model->username,
);
$this->menu=array(
	array('url'=>url('/user/admin'), 'label'=>'Usuarios','active'=>true,),
	array('url'=>url('/rights'), 'label'=>'Permisos','active'=>false,),
	array('url'=>url('/menu'), 'label'=>'Menu','active'=>false,),
);
?>

<?php echo $this->renderPartial('_menu', array(
		'list'=> array(),
	)); 
?>
</div>
</div>
<div class="row-fluid"><div class="span2-fluid">
<h3><?php echo "View user: ".$model->username;?></h3>
</div>
<div class="span8-fluid">
<?php
echo CHtml::link(UserModule::t('Update User'),array('update','id'=>$model->id),array('class'=>'btn'));
echo "&nbsp;";
echo CHtml::linkButton(UserModule::t('Delete User'),array('submit'=>array('delete','id'=>$model->id),'confirm'=>UserModule::t('Are you sure to delete this item?'),'class'=>'btn btn-danger'));
?>
</div>
</div>
<div class="row-fluid">
<div class="span12-fluid">
<?php
	$attributes = array(
		'id',
		'username',
	);
	
	$profileFields=ProfileField::model()->forOwner()->sort()->findAll();
	if ($profileFields) {
		foreach($profileFields as $field) {
			array_push($attributes,array(
					'label' => UserModule::t($field->title),
					'name' => $field->varname,
					'type'=>'raw',
					'value' => (($field->widgetView($model->profile))?$field->widgetView($model->profile):(($field->range)?Profile::range($field->range,$model->profile->getAttribute($field->varname)):$model->profile->getAttribute($field->varname))),
				));
		}
	}
	
	array_push($attributes,
		'password',
		'email',
		'activkey',
		array(
			'name' => 'createtime',
			'value' => date("d.m.Y H:i:s",$model->createtime),
		),
		array(
			'name' => 'lastvisit',
			'value' => (($model->lastvisit)?date("d.m.Y H:i:s",$model->lastvisit):UserModule::t("Not visited")),
		),
		array(
			'name' => 'superuser',
			'value' => User::itemAlias("AdminStatus",$model->superuser),
		),
		array(
			'name' => 'status',
			'value' => User::itemAlias("UserStatus",$model->status),
		)
	);
	
	$this->widget('zii.widgets.CDetailView', array(
		'data'=>$model,
		'htmlOptions'=>array(
			'class' =>'table table-bordered table-striped table-condensed',
			),
		'attributes'=>$attributes,
	));
	

?>
</div>
</div>